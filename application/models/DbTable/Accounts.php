...
class Application_Model_DbTable_Accounts extends Zend_Db_Table_Abstract
{
	...
	public function getFreelanceDetails($array,$account_id)
	{
		$select = $this->select()->setIntegrityCheck(false)
				->from($this,$array)
				->joinLeft(array('f'=>'freelancers'),'f.account_id = '.$this->_name.'.id',array('*'))
				->joinLeft(array('t'=>'transactions'),'t.account_id = '.$this->_name.'.id',array('payment_status','added'))
				->where($this->_name.'.id = ?',$account_id);
				//echo $select;die;
		$row = $this->fetchRow($select);
		if($row!=NULL)
		{
