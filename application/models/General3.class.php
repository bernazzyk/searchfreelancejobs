...
class Application_Model_General extends Application_Model_Freelancer
{
	...
	public function getNrOfProjects()
	{
		...
		$sql_stmt = "SELECT COUNT(id) from projects";
...
and id not in (select project_id as id from proposals where accepted = 1)
	...
	*/
	...
	public function getNrOfFreelancers()
	{
		...
		$sql_stmt = "SELECT COUNT(*) FROM freelancers";
		$SumMoney = $this->db->fetchOne( $sql_stmt );
		return $SumMoney;
	}
	...
	public function InsertProjects($ProjectsList, $platform_id, $ProjectFiles = array()) 
	{
		...
		if($result)
		{
			...
			unset($result);
					   ...
					   $sql_stmt_to_insert = 'INSERT INTO `projects` (`url`,`external_url`, `external_id`, `title`, `description`, `posted`, `ends`, `budget_low`, `budget_high`, `platform_id`, `active`, `jobtype`,`bids`, `bids_avg`, `budget_currency`,`external_user_id`) VALUES';
		$sql_stmt_to_insert_on_update = 'INSERT INTO `projects` (`id`,`url`,`external_url`, `external_id`, `title`, `description`, `posted`, `ends`, `budget_low`, `budget_high`, `platform_id`, `active`, `jobtype`,`bids`, `bids_avg`, `budget_currency`,`external_user_id`) VALUES';
		...
		{ 
			...
			$sql_values = "'{$url}','$Values[external_url]','$Values[external_id]','$Values[title]','$Values[description]','$Values[posted]','$Values[ends]',".$Values['budget_low'].','.$Values['budget_high'].','.$platform_id.','.$Values['active'].','. $Values['jobtype'].','.$Values['bids'].','.$Values['bids_avg'].','.$Values['budget_currency'].','."'$Values[external_user_id]'";
	...
	{
		$sql_stmt = "SELECT * FROM curl_platforms_users_data WHERE user_id={$user_id} AND platform_id={$platform_id}";
		return $this->db->fetchRow( $sql_stmt );
	}
	...
	public function insertApiPlatformUser($platform_id, $user_id, $access_token=null ,$access_token_secret=null)
	{
		...
		$exist_record =  $this->db->fetchOne( $sql_stmt );
		if($exist_record) {
			$update_stmt = "UPDATE `curl_platforms_users_data` SET `access_token` = '{$access_token}', `access_token_secret` = '{$access_token_secret}', connected = 1  WHERE `user_id`={$user_id} AND `platform_id`={$platform_id}";
			$this->db->query( $update_stmt );
		} else {
			$insert_stmt = "INSERT INTO `curl_platforms_users_data`(`platform_id`,`user_id`,`access_token`,`access_token_secret`,`connected`) VALUES({$platform_id},{$user_id},'{$access_token}','{$access_token_secret}',1)";
			$this->db->query( $insert_stmt );
		}
	}
	...
	public function insertRemotePlatformUser($platform_id, $user_id, $username,$password)
	{
		...
		$exist_record =  $this->db->fetchOne( $sql_stmt );
		if($exist_record) {
			$update_stmt = "UPDATE `curl_platforms_users_data` SET `username` = '{$username}', `password` = '{$password}', connected = 1 WHERE `user_id`={$user_id} AND `platform_id`={$platform_id}";
			$this->db->query( $update_stmt );
		} else {
			$insert_stmt = "INSERT INTO `curl_platforms_users_data`(`platform_id`,`user_id`,`username`,`password`,`connected`) VALUES({$platform_id},{$user_id},'{$username}','{$password}',1)";
			$this->db->query( $insert_stmt );
		}
	}
	...
	{
		$sql_stmt = "SELECT pud.*, p.is_curl FROM curl_platforms_users_data as pud, platforms AS p WHERE pud.user_id={$user_id} AND pud.connected = 1 AND pud.platform_id = p.id";
		return $this->db->fetchAll($sql_stmt );
	}
